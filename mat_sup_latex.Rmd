...
\begin{table}[!ht]
    \caption{Variable descriptions for Table S1. Available at https://irene-alcocer.github.io/Acoustic-Indices/}
\end{table}
...
descript_acous_data <- c("ACI, AEI, ADI, AR, BIO, H, H\\textsubscript{t}, H\\textsubscript{f}, M, NP, or NDSI",
...
kbl(features_tbl, booktabs = TRUE, escape = FALSE,
    caption = "List of the 34 features used to characterize studies that tested the relationship between acoustic indices and diversity metrics.") %>%
...
\begin{figure}[H]
    ...
    \caption{
        Pseudoreplication summary. The data represent the total number of articles for each index. Orange dots represent the number of  studies with pseudoreplicates studies and green are without pseudoreplication studies. One article (Papin et al., 2019) was withdrawn from the pseudoreplication analysis as it was impossible to obtain data on pseudoreplication, thus leading to some variation in the total for each acoustic index. See Table 2 for definitions of acoustic indices.
    }
\end{figure}
...
kbl(r_main_pander, booktabs = TRUE, row.names = FALSE,
    caption = "
        Estimates from intercept-only model converted to Pearson's correlation coefficient \\textit{r}. \"Estimate\" is the Pearson's \\textit{r} summary effect size. CI.lb and CI.ub are the confidence interval lower and upper bounds, respectively.
    ") %>%
    kable_styling(latex_options = c("striped", "HOLD_position"),
                                    full_width = FALSE) 
```
...
kbl(df_pred_indices, booktabs = TRUE, 
    caption = "
        Estimates from the sub-group analysis. The \"Estimate\" column is the Pearson correlation effect size \\textit{r}; SE is the standard error and CI.lb and CI.ub; the lower and upper bounds of the confidence intervals, respectively. See Table 2 for definitions of acoustic indices.
    ") %>%
  kable_styling(latex_options = c("striped", "HOLD_position"), full_width = FALSE) 
```
...
```{r subgroup_plt, fig.pos = "H", fig.cap = "Effect size mean estimates (circles) and corresponding 95% confidence intervals (horizontal lines) obtained from sub-group meta-analysis with acoustic indices as the moderating factor. Estimated effect sizes whose 95% confidence intervals do not overlap zero (black vertical line) indicate a positive correlation between acoustic indices and diversity if they are to the right of zero, or a negative correlation if they are to the left of zero. The dashed green vertical line represents the summary effect size obtained from the intercept only meta-analysis."}
...
kbl(vif_meta, booktabs = TRUE, 
    caption = "
        VIF values obtained for each moderator level.See Table 2 for definitions of acoustic indices.
    ") %>%
  kable_styling(latex_options = c("striped", "HOLD_position"), full_width = FALSE) 
```
...
```{r check_outliers, fig.pos = "H", fig.cap = "Boxplot and distribution of effect size values (dots) of the two studies identified as outliers. The x-axis shown the to the Pearson \\textit{r} effect size. The green vertical dashed line is the summary effect obtained in the intercept-only model."}
...
```{r effect_size_impact_factor, fig.pos = "H", fig.cap = "Relationship between effect size mean values (circles) and journal impact factor. Circle size indicates the relative sample size for each effect size. The fitted line is a simple least squares regression with the corresponding 95% confidence interval region shaded grey. The dashed horizontal line represents an effect size of 0. Effect size mean values are positioned along the impact factor axis with minor random noise to reduce overlapping."}
plt_color <- "deeppink4"
